...
public class BST<T> {
	...
	public T getRoot() throws NoSuchElementException {
		if (root == null) {
			throw new NoSuchElementException("getRoot: BST should not be empty.");
		}
		return root.data;
	}
	...
	public T findMin() throws NoSuchElementException {
		if (root == null) {
			throw new NoSuchElementException("findMin: BST should not be empty");
		}
		return findMin(root);
	}
	...
	public T findMax() throws NoSuchElementException {
		if (root == null) {
			throw new NoSuchElementException("findMax: BST should not be empty.");
		}
		return findMax(root);
	}
	 ...
	 * @param c    the Comparator indicating how data in the tree is ordered
	 */
	public void insert(T data, Node node, Comparator<T> c) {
		if (c.compare(data, node.data) <= 0) {
			if (node.left == null) {
	...
	private Node remove(T data, Node node, Comparator<T> c) {
		...
		else if (c.compare(data, node.data) < 0) // 2. Otherwise, if value < the node's data
		{
